...
// ---------------------------------------------------------------------------
// Menu data — teal accent untuk dosen
// ---------------------------------------------------------------------------
const menuGroups: MenuGroup[] = [
    {
        title: "MAIN",
        items: [
            { label: "Dashboard", href: "/dosen", icon: LayoutDashboard, exact: true },
        ],
    },
    {
        title: "STATISTIK DOSEN",
        items: [
            { label: "Grafik & Analisis", href: "/dosen/statistik", icon: BarChart2 },
        ],
    },
];
...
export default function SidebarDosen({
    ...
    // Persist theme
    useEffect(() => {
        const savedTheme = localStorage.getItem("sidebar-theme-dosen") as "dark" | "light" | null;
    ...
    useEffect(() => {
        if (!mounted) return;
        localStorage.setItem("sidebar-theme-dosen", theme);
                ...
                >
                    {!collapsed && (
                        <div className="flex min-w-0 flex-1 items-center gap-3">
                            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-600 text-sm font-extrabold text-white shadow-lg shadow-teal-600/30">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="mt-0.5 truncate text-xs font-medium text-teal-600 dark:text-teal-400">
                                    Portal Dosen
                                </p>
                            </div>
                        </div>
                    ...
                    {!collapsed ? (
                        <>
                            <div className="flex items-center rounded-xl bg-slate-50 p-3 dark:bg-white/5">
                                <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-teal-600 font-bold text-white">
                                    {initialLetter}
                        ...
                        <div className="flex flex-col items-center gap-2">
                            <div className="group relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-teal-600 font-bold text-white">
                                {initialLetter}
                                <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white bg-emerald-500 dark:border-[#0B1220]" />
                                <span className="pointer-events-none absolute left-full top-1/2 z-50 ml-3 -translate-y-1/2 whitespace-nowrap rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white opacity-0 shadow-xl transition-opacity duration-200 group-hover:opacity-100 dark:bg-slate-800">
                                    {displayName}
