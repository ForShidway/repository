...
const menuGroups: MenuGroup[] = [
    {
        title: "MAIN",
        items: [{ label: "Beranda", href: "/admin", icon: LayoutDashboard }],
    },
    {
        title: "KELOLA DATA",
        items: [
            { label: "Tugas Akhir", href: "/admin/tugas-akhirs", icon: FileText },
    ...
    {
        title: "SISTEM",
        items: [{ label: "Pengguna", href: "/admin/users", icon: UserCog }],
    },
];

// ---------------------------------------------------------------------------
// Props — collapsed & mobileOpen are CONTROLLED by the parent layout.
// This is required so the page content next to the sidebar can react
// (e.g. shift its margin-left) when the sidebar expands/collapses.
// ---------------------------------------------------------------------------
...
export default function AdminSidebar({
    ...
    // Theme is local UI state (doesn't affect layout width), so it's fine
    // to keep it inside the sidebar itself.
    const [theme, setTheme] = useState<"dark" | "light">("dark");
    const [mounted, setMounted] = useState(false);

    // Restore persisted theme on mount (client-only)
    useEffect(() => {
        const savedTheme = localStorage.getItem("sidebar-theme") as "dark" | "light" | null;
    ...
    return (
        <>
            ...
            >
                ...
                >
                    {!collapsed && (
                        <div className="flex min-w-0 flex-1 items-center gap-3">
                            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-600 text-sm font-extrabold text-white shadow-lg shadow-indigo-600/30">
                ...
                {/* Collapsed-state toggle (own row, centered) */}
                {collapsed && (
                        ...
                        <div key={group.title} className="mb-6">
                            ...
                            <div className="space-y-1">
                                {group.items.map((item) => {
                                    // Exact match for root /admin to avoid always being active on sub-pages
                                    const active =
                                        item.href === "/admin"
                                            ? pathname === "/admin"
                    ...
                    {!collapsed ? (
                        <>
                            <div className="flex items-center rounded-xl bg-slate-50 p-3 dark:bg-white/5">
                                <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-600 font-bold text-white">
                                    A
                                ...
                                <div className="ml-3 min-w-0">
                                    ...
                                    <p className="truncate text-xs text-slate-500 dark:text-slate-400">Super Admin</p>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col items-center gap-2">
                            <div className="group relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-600 font-bold text-white">
                                A
                                <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white bg-emerald-500 dark:border-[#0B1220]" />
                                <span className="pointer-events-none absolute left-full top-1/2 z-50 ml-3 -translate-y-1/2 whitespace-nowrap rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white opacity-0 shadow-xl transition-opacity duration-200 group-hover:opacity-100 dark:bg-slate-800">
                                    Administrator
