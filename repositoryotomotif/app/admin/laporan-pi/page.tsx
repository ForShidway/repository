...
export default function LaporanPIPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try { 
                const response = await fetch("/api/laporanPi");
    ...
    async function handleDelete(id: number) {
        const confirmed = window.confirm(
            "Apakah Anda yakin ingin menghapus data Laporan Pelatihan Industri ini?"
        );
        if(!confirmed) {
            return;
        }
        try{
            const response = await fetch(`/api/laporanPi/${id}`,{
                method: "DELETE"
            })
            const result = await response.json();
            if (!response.ok) {
        ...
        <main className="page-shell">
            <section className="page-heading">
                <div>
                    ...
                    <h1>
                        Kumpulan Data Laporan Pelatihan Industri
                    </h1>
                                ...
                                {data.map((item, index) => (
                                    <tr key={(item.id)}>
                                        ...
                                        <td className="action-cell">
                                            <button onClick={() => router.push(`/admin/laporan-pi/${item.id}`) }  className="detail-button"  >
                                                Lihat Detail
                                            </button>
