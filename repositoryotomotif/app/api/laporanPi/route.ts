...
export async function GET() {
    try {
        const laporanPis = await prisma.laporanPi.findMany({
            orderBy: { createdAt: "desc" },
            include: { dosenPembimbing: true },
        });

        return NextResponse.json(laporanPis);
    } catch (error) {
        console.error("Error fetching laporan Pi:", error);
        return NextResponse.json(
            { error: "Gagal mengambil laporan Pi" },
            { status: 500 },
        );
    }
}
...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (Number.isNaN(tanggalMulai.getTime()) || Number.isNaN(tanggalSelesai.getTime())) {
            return NextResponse.json(
                { message: "Format periode magang ini tidak valid" },
                { status: 400 },
            );
        }
        ...
        if (file) {
            ...
            filePath = `/uploads/laporan-Pi/${uniqueFileName}`;
        }

        const laporanPi = await prisma.laporanPi.create({
            data: {
                name,
                nim,
                namaInstansi,
        ...
        return NextResponse.json(laporanPi, { status: 201 });
    } catch (error) {
        if (savedFilePath) {
            await fs.unlink(savedFilePath).catch(() => undefined);
        }
        console.error("Error creating Laporan Pi:", error);
        return NextResponse.json(
            { error: "Gagal membuat Laporan Pi" },
            { status: 500 },
        );
    }
}